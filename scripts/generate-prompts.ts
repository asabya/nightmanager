...
function parseCanonical(role: Role): CanonicalPrompt {
  const raw = readFileSync(join(agentsDir, `${role}.md`), "utf-8").replace(/\r\n/g, "\n");
...
function constName(role: Role): string {
  return `${role.toUpperCase()}_CANONICAL`;
}

function renderGeneratedTs(prompts: CanonicalPrompt[]): string {
  const header = [
...
function renderClaudeAgent(prompt: CanonicalPrompt): string {
...
function main(): void {
  const prompts = ROLES.map(parseCanonical);
