...
type OracleWebSearchInput = Static<typeof oracleWebSearchSchema>;
...
type WebSearchModule = {
  search: (query: string, options?: {
    provider?: OracleWebSearchInput["provider"];
    numResults?: number;
    includeContent?: boolean;
    recencyFilter?: OracleWebSearchInput["recencyFilter"];
    domainFilter?: string[];
    signal?: AbortSignal;
  }) => Promise<{
    answer?: string;
    ...
    inlineContent?: Array<{ title?: string; url?: string; content?: string; error?: string | null }>;
  }>;
};

type CodeSearchModule = {
  executeCodeSearch: (
    toolCallId: string,
...
async function loadWebSearchModule(): Promise<WebSearchModule> {
  try {
    const moduleName = PI_WEB_ACCESS_SEARCH_MODULE;
    return await import(moduleName) as WebSearchModule;
  } catch {
...
async function callPiWebAccessExaMcp(toolName: string, args: Record<string, unknown>, signal?: AbortSignal): Promise<string> {
...
function parsePiWebAccessMcpResults(text: string): McpParsedResult[] | null {
  ...
  const parsed = blocks.map(block => {
    ...
    if (textStart >= 0) {
      ...
      if (highlights?.index != null) content = block.slice(highlights.index + highlights[0].length).trim();
    }
...
function buildFallbackWebQuery(query: string, options: Parameters<WebSearchModule["search"]>[1]): string {
  const parts = [query];
  ...
  if (options?.recencyFilter) {
    const now = new Date();
    const recencyHints: Record<NonNullable<OracleWebSearchInput["recencyFilter"]>, string> = {
      day: "past 24 hours",
      week: "past week",
...
async function searchWithPiWebAccessExaMcp(query: string, options: Parameters<WebSearchModule["search"]>[1] = {}) {
  ...
  if (!parsed) return { answer: text, provider: "exa", results: [] };
  return {
    provider: "exa",
    answer: parsed
...
async function executePiWebAccessCodeSearchFallback(
  _toolCallId: string,
  params: { query: string; maxTokens?: number },
  signal?: AbortSignal,
): ReturnType<CodeSearchModule["executeCodeSearch"]> {
...
function normalizeQueries(params: OracleWebSearchInput): string[] {
...
function formatOracleWebSearchResult(args: {
  ...
  if (args.provider) lines.push(`Provider: ${args.provider}`);
  if (args.error) {
  ...
  if (inlineContent.length > 0) {
    ...
    for (let i = 0; i < inlineContent.length; i++) {
      ...
      if (item.content) lines.push(truncateForTool(item.content, 3000));
    }
  }

  if (!answer && results.length === 0 && inlineContent.length === 0) lines.push("No results returned.");
  return lines.join("\n");
}
...
const oracleWebSearchTool = defineTool({
  ...
  renderResult(result, { expanded }, theme) {
    const details = result.details as { queryCount?: number; successfulQueries?: number; totalResults?: number; error?: string } | undefined;
  ...
  async execute(_toolCallId, params: OracleWebSearchInput, signal, onUpdate) {
    ...
    try {
      ...
      const details: Array<{ query: string; provider?: string; sourceCount: number; error?: string }> = [];
      let totalResults = 0;
...
const oracleCodeSearchTool = defineTool({
  ...
  renderResult(result, { expanded }, theme) {
    const details = result.details as { maxTokens?: number; error?: string } | undefined;
    if (details?.error) return new Text(theme.fg("error", `Error: ${details.error}`), 0, 0);
  ...
  async execute(toolCallId, params: OracleCodeSearchInput, signal) {
    const query = params.query.trim();
    if (!query) {
      return {
        content: [{ type: "text", text: "Error: No query provided." }],
