...
let setPendingEventsFn = null;
let getPendingEventsFn = null;
...
const { mockAgentClass } = vi.hoisted(() => {
    ...
    const createAgentInstance = function () {
        const handlers = [];
        return {
            _handlers: handlers,
...
describe("runIsolatedSubagent", () => {
    ...
    beforeEach(() => {
        ...
        mockAgentClass.mockImplementation(function () {
            const instanceHandlers = [];
            // Track messages from events for getState to return
            let eventMessages = [];
            let finalTextFromEvents = "";
            const instance = {
                prompt: vi.fn().mockImplementation(async function (_userMsg) {
                ...
                subscribe: vi.fn().mockImplementation((handler) => {
                    ...
                    instance._handlers = instanceHandlers;
                    return () => {
                        const idx = instanceHandlers.indexOf(handler);
                        if (idx >= 0)
                            instanceHandlers.splice(idx, 1);
                    };
                }),
        ...
        let testPendingEvents = [];
        setPendingEventsFn = (events) => { testPendingEvents = events; };
        getPendingEventsFn = () => testPendingEvents;
    });
    // Helper to extract messages from events for mock's getState
    function extractMessagesFromEvents(events) {
        const messages = [];
        for (const event of events) {
    ...
    function extractFinalTextFromEvents(events) {
    ...
    async function emitEvents(agent, events) {
            ...
            const events = [
                {
                    ...
                    assistantMessageEvent: { type: "content_delta", delta: "Hello " },
                },
                {
    ...
    describe("live transcript updates", () => {
        ...
        it("avoids duplicate assistant text from message_update and message_end", async () => {
            ...
            const assistantTextEntries = result.details.entries.filter((e) => e.type === "assistant_text");
            ...
            expect(finalAssistantEntry.text).toBe(fullText);
            expect(assistantTextEntries.filter((e) => e.text === fullText)).toHaveLength(1);
        });
        it("verifies live update content contains text and is not empty during execution", async () => {
            const contentPerUpdate = [];
            const onUpdate = vi.fn((update) => {
            ...
            const hasNonEmptyContent = contentPerUpdate.some(content => content.length > 0 && content.some(text => text.length > 0));
        ...
        it("handles message_update without delta by reading all text blocks from content", async () => {
            ...
            const entries = onUpdate.mock.calls.flatMap(call => call[0].details.entries.filter((e) => e.type === "assistant_text"));
        ...
        it("final assistant transcript text matches result.finalText on completion", async () => {
            ...
            const events = [
                {
                    ...
                    assistantMessageEvent: { type: "content_delta", delta: "Working..." },
                },
                {
                    type: "message_end",
                    message: {
            ...
            expect(finalAssistantText.text).toBe(result.finalText);
        });
        it("no duplicate full assistant text entries when streaming already captured same text", async () => {
            ...
            const events = [
                {
                    ...
                    assistantMessageEvent: { type: "content_delta", delta: fullText },
                },
                {
                    type: "message_end",
                    message: {
            ...
            const assistantEntries = result.details.entries.filter((e) => e.type === "assistant_text");
            expect(assistantEntries.length).toBeGreaterThan(0);
            expect(assistantEntries.filter((e) => e.text === fullText)).toHaveLength(1);
            expect(assistantEntries[assistantEntries.length - 1].text).toBe(fullText);
        });
    });
