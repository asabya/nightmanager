...
import { createTranscriptState, appendStatus, appendAssistantText, appendToolCall, appendToolResult, finalizeTranscriptDetails, } from "../../src/core/transcript.js";
describe("transcript", () => {
    describe("createTranscriptState", () => {
        it("should create initial state with tool and task", () => {
            const state = createTranscriptState("finder", "test task");
    ...
    describe("appendAssistantText - streaming merge", () => {
        it("should merge streamed assistant text into one chronological entry", () => {
            const state = createTranscriptState("worker", "test task");
        ...
        it("should NOT merge when previous entry streaming is false", () => {
            ...
            expect(updated.entries[0].streaming).toBe(false);
            expect(updated.entries[1].text).toBe("world!");
            expect(updated.entries[1].streaming).toBe(true);
        });
    });
    describe("appendToolCall and appendToolResult", () => {
        it("should record tool calls and results in chronological order", () => {
            let state = createTranscriptState("worker", "test task");
            // Add tool call
    ...
    describe("chronology preservation", () => {
        ...
        it("should preserve chronology for streaming merged entries", () => {
            ...
            expect(state.entries[1].text).toBe("stream part 1stream part 2");
        ...
        it("should NOT merge streaming text across intervening entries (chronological previous check)", () => {
            ...
            expect(state.entries).toHaveLength(3);
            expect(state.entries[0].timestamp).toBe(2000);
            expect(state.entries[0].text).toBe("Hello "); // Not merged!
            expect(state.entries[1].timestamp).toBe(2500);
            expect(state.entries[2].text).toBe("middle "); // New entry, not merged
            ...
            expect(state.entries[0].timestamp).toBe(2000);
            expect(state.entries[0].text).toBe("Hello "); // Still separate from 2000
            expect(state.entries[1].timestamp).toBe(2500);
            expect(state.entries[2].timestamp).toBe(3000);
            expect(state.entries[2].text).toBe("middle world!"); // Merged!
        });
    });
});
