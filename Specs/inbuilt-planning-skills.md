# Spec: Inbuilt planning skills for subagents
...
Right now, the planning workflows that help humans move from rough ideas to implementation-ready work are spread across prompts, docs, and external conventions. The repo already has strong Night Shift execution workflows, but the plan-to-spec and plan-to-todos steps are not packaged as first-class Pi skills that are immediately available after installing `subagents`.
...
- Make `grill-me`, `to-prd`, and `to-issues` available as inbuilt Pi skills after installing `subagents`.
...
- Keep Night Shift operating on local repo artifacts, not GitHub as the primary source of truth.

## Non-Goals

- Reworking the Night Shift execution model.
...
The Night Shift workflow continues to depend on local `TODOs.md` and `Specs/` files.

## Desired Behavior

After installing `subagents`, Pi should discover three planning skills automatically with no extra user setup:
...
- Keeps Night Shift compatibility by preserving the same TODO structure and status semantics used today.
...
- The repo must remain compatible with Night Shift reading local `TODOs.md` entries even when planning starts from a skill flow.
...
- Package the three planning skills so Pi can discover them as part of the installed `subagents` extension.
...
- Preserve the local-file-first model so Night Shift can continue to operate without depending on GitHub issues.
...
- Update examples so users know how to invoke `grill-me`, `to-prd`, and `to-issues` after installing `subagents`.
